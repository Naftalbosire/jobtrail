...
export function PanelProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    applicationsApi.getAll().then(setApps).catch(console.error);
  }, []);
  ...
  const saveApp = async () => {
    if (!draft?.company.trim()) return;
    if (isNew) {
      const created = await applicationsApi.create(draft);
      setApps(prev => [created, ...prev]);
    } else {
      const updated = await applicationsApi.update(draft.id, draft);
      setApps(prev => prev.map(a => a.id === draft.id ? updated : a));
    }
    closePanel();
  };
  ...
  const handleFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !uploadingFor || !draft) return;
    if (draft.id) {
      const att = await applicationsApi.uploadAttachment(draft.id, file, uploadingFor);
      setDraft(prev => ({
        ...prev,
        attachments: uploadingFor === "other"
          ? [...prev.attachments, att]
